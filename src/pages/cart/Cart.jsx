  ...
  return (
    <div className="min-h-screen bg-blackDeep text-primary pb-20 pt-10">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        {/* HEADER */}
        <header className="mb-12 border-b border-steel/30 pb-6 flex justify-between items-end">
          <h1 className="text-4xl md:text-5xl font-light tracking-tighter">
            Tu{" "}
            <span className="text-gold font-normal">Carrito  </span>
            
          </h1>
          <ShoppingCart size={40 } />
          
          
          <span className="text-ice text- hidden md:block">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 xl:gap-20">
          ...
          <div className="lg:col-span-8 space-y-0 divide-y divide-steel/20">
            ...
            {detailsOrders.map((item) => (
              <div key={item.idDetailsOrder} className="py-8 group">
                <div className="flex flex-col sm:flex-row gap-6">
                  {/* Placeholder visual para servicios sin imagen */}
                  <div className="w-24 h-24 sm:w-32 sm:h-32 bg-graphite flex items-center justify-center text-steel/40 border border-steel/10 rounded-sm shrink-0">
                    <Icons.Tag />
                  </div>
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="text-lg font-medium text-primary tracking-wide">
                          Servicio #{item.idService ?? item.idGarmentService}
                        </h3>
                      ...
                      <div className="flex flex-wrap gap-3 text-sm text-ice/80 mb-3">
                        <span className="bg-steel/20 px-2 py-1 rounded text-xs uppercase tracking-wider">
                          {item.selectColor}
                        </span>
                        <span className="bg-steel/20 px-2 py-1 rounded text-xs uppercase tracking-wider">
                          {item.selectSize}
                      ...
                      {item.details && (
                        <p className="text-steel text-sm italic border-l-2 border-steel/30 pl-3 py-1">
                          "{item.details}"
                        </p>
                      )}
                    </div>
            ...
            {customs.map((custom) => (
              <div key={custom.idCustom} className="py-8 group">
                <div className="flex flex-col sm:flex-row gap-6">
                  {/* Imagen del producto */}
                  <div className="w-full sm:w-40 aspect-square bg-graphite rounded-sm overflow-hidden border border-steel/20 shrink-0 relative">
                    ...
                    <div className="absolute top-2 left-2 bg-gold text-blackDeep text-[10px] font-bold px-2 py-1 uppercase tracking-wider">
                      Custom
                    </div>
                  </div>
